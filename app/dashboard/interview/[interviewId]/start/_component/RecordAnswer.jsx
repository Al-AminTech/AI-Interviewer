...
function RecordAnswer({
  ...
  const UpdateUserAnswer = async () => {
    try {
      ...
      // Chat session logic here (ensure mockJsonResp is defined correctly)
      const feedbackPrompt =
        "Question:" +
        mockInterviewQuestion[activeQuestionIndex]?.question +
        ", User Answer:" +
        userAnswer +
        ", Depends on question and user answer, please give a rating and feedback in JSON format with 'rating' and 'feedback' fields.";
      
      const result = await chatSession.sendMessage(feedbackPrompt);
      const mockJsonResp = result.response.text()
        .replace("```json", "")
        .replace("```", "");
      console.log("Received Mock JSON Response:", mockJsonResp);
      ...
      console.error("Error while recording answer:", error);
      toast("An error occurred while recording the answer");
    } finally {
      setLoading(false);
    }
